...
public class CommandRemoveForbiddenWord implements CommandInterface {
    @Override
        ...
        return Mono.justOrEmpty(event.getInteraction().getMember())
                ...
                .flatMap(hasPermission -> {
                    ...
                    if (hasPermission) {
                        ...
                        if (ForbiddenWordList.removeWord(wordOption, guild.getId().asLong())) {
                            return event.reply()
                                    .withEphemeral(true)
                                    .withContent("Removing \"" + wordOption + "\" from the list");
                        } else {
                            return event.reply()
                                    .withEphemeral(true)
                                    .withContent("The word was not in the list");
                        }
                    } else {
